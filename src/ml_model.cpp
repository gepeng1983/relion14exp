...
void MlModel::initialise()
{
	...
	//
    hrise.resize(nr_classes);
    hturn.resize(nr_classes);
    // Set --hrise <= 0 to suppress the following
    // initialisation of helical parameters
    if (inihrise > 0)
    {

        for (int iclass=0; iclass<nr_classes; iclass++)
        {
                hrise[iclass]=inihrise;
                hturn[iclass]=inihturn;
        }
    }
...
void MlModel::read(FileName fn_in)
{
	...
	FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDclass)
	{
		if (!MDclass.getValue(EMDL_MLMODEL_REF_IMAGE, fn_tmp) ||
			...
			!MDclass.getValue(EMDL_MLMODEL_HELIX_RISE, hrise[iclass]) ||
			!MDclass.getValue(EMDL_MLMODEL_HELIX_TURN, hturn[iclass]) 	)
...
void MlModel::write(FileName fn_out, HealpixSampling &sampling, bool do_write_bild)
{
	...
	for (int iclass = 0; iclass < nr_classes; iclass++)
	{
		...
		MDclass.setValue(EMDL_MLMODEL_HELIX_RISE, hrise[iclass]);
		MDclass.setValue(EMDL_MLMODEL_HELIX_TURN, hturn[iclass]);
	...
	if (fn_ref != "None")
	{
		...
		if (fn_ref.isStarFile())
		{
			...
			FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDref)
			{
				...
				Iref.push_back(img());
				DOUBLE t;
				MDref.getValue(EMDL_MLMODEL_HELIX_RISE, t);
				hrise.push_back(t);
				MDref.getValue(EMDL_MLMODEL_HELIX_TURN, t);
				hturn.push_back(t);
				nr_classes++;
			}
		}
...
#define MAX_PACK_SIZE 200000000L
#endif
...
void MlWsumModel::pack(MultidimArray<DOUBLE> &packed, int &piece, int &nr_pieces, bool do_clear)
{
    ...
    {
        idx_start = ((long long int) piece) * MAX_PACK_SIZE;
