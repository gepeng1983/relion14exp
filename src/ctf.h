...
public:
    ...
    void setValues(DOUBLE _defU, DOUBLE _defV, DOUBLE _defAng,
    		DOUBLE _voltage, DOUBLE _Cs, DOUBLE _Q0, DOUBLE _PSh, DOUBLE _Bfac, DOUBLE _scale = 1.);
	...
	DOUBLE argument_psh = argument - PSh;
        DOUBLE retval;
        if (do_intact_until_first_peak && ABS(argument_psh) < PI/2.)
        {
        	retval = 1.;
        }
        else
        {
            ...
            retval = -(K3*sin(argument_psh) - Q0*cos(argument)); // Q0 should be positive
        }
        if (do_damping)
        {
