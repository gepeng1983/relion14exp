...
int MpiNode::relion_MPI_Bcast(void *buf, std::ptrdiff_t count, MPI_Datatype datatype, int root, MPI_Comm comm)
{
     	 int result;
   	 MPI_Request request;
   	 DOUBLE current_time = MPI_Wtime();
        ...
        if (totalsize <= blocksize ) {
            result = MPI_Bcast(buf, count, datatype, root, comm);
	    if (result != MPI_SUCCESS)
	    {
		report_MPI_ERROR(result);
	    } 
            ...
            std::ptrdiff_t i(0);
            for(; i<ntimes; ++i) {
	        result = MPI_Bcast(buffer+i*blocksize, blocksize, MPI_CHAR, root, comm);
                if (result != MPI_SUCCESS) {
                    report_MPI_ERROR(result);
                }
            }
            if(nremain>0) {
	        result = MPI_Bcast(buffer+i*blocksize, nremain, MPI_CHAR, root, comm);
                if (result != MPI_SUCCESS) {
                    report_MPI_ERROR(result);
                }
            }
