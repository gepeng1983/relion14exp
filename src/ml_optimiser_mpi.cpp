	...
	for (int iclass = 0; iclass < mymodel.nr_classes; iclass++)
    {
		if (mymodel.pdf_class[iclass] > 0.)
		{
			...
			if (node->rank == reconstruct_rank1)
			{
				...
				if (helix && (! do_split_random_halves))
				...
				if (htune && !(iter == 1 && do_firstiter_cc))
				{
					...
					//hsearch(model, verb, half#
					hsearch(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], 1, fn.c_str());
					if (do_split_random_halves)
					{
					//TODO	hsync();
						DOUBLE hturn2, hrise2;
						MPI_Status status;

						node->relion_MPI_Recv(&hrise2, 1, MY_MPI_DOUBLE, 2, 0, MPI_COMM_WORLD, status);
						node->relion_MPI_Recv(&hturn2, 1, MY_MPI_DOUBLE, 2, 0, MPI_COMM_WORLD, status);

						std::cout << " Helix: class (" << iclass << ") my hrise/hturn: " << mymodel.hrise[iclass] << "/" << mymodel.hturn[iclass] << " rank2: " << hrise2 << "/" << hturn2 << std::endl;
						
						// if do_join_random_halves, the reconstruction in rank2 was not done - so the volume is the
						// reference of last iteration, we should not take its hsearch into account
						if (!do_join_random_halves) {
							mymodel.hrise[iclass]=(hrise2+mymodel.hrise[iclass])/2.;
							mymodel.hturn[iclass]=(hturn2+mymodel.hturn[iclass])/2.;
						}
						else
							std::cout << "  +Random halves joined, not combining hsearch results (ignoring rank2 helicity)." << std::endl;
						
						node->relion_MPI_Send(&(mymodel.hrise[iclass]), 1, MY_MPI_DOUBLE, 2, 0, MPI_COMM_WORLD);
						node->relion_MPI_Send(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, 2, 0, MPI_COMM_WORLD);
						//std::cout << " Helix: averaged hrise/hturn: " << hrise << "/" << hturn << std::endl;
					}
				}
				if (helix) 
					himpose(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], 1);

				if (test2>0)
					test1(mymodel.Iref[iclass], test2, mymodel.pixel_size);
			...
			if (do_split_random_halves)
			{
				...
				if (node->rank == reconstruct_rank2)
				{
					...
					if (htune && !(iter == 1 && do_firstiter_cc))
					{
						...
						//hsearch(model, verb, half#
						hsearch(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], 0, fn.c_str());
					//TODO	hsync();
						node->relion_MPI_Send(&(mymodel.hrise[iclass]), 1, MY_MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
						node->relion_MPI_Send(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);

						//std::cout << " Helix: rank2 my hrise/hturn: " << hrise << "/" << hturn << std::endl;

						MPI_Status status;
						node->relion_MPI_Recv(&(mymodel.hrise[iclass]), 1, MY_MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, status);
						node->relion_MPI_Recv(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, status);

						//std::cout << " Helix: rank2 averaged hrise/hturn: " << hrise << "/" << hturn << std::endl;
	
					}
					if (helix) 
						himpose(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], 0);
...
//#define DEBUG_RECONSTRUCT
	...
	for (int iclass = 0; iclass < mymodel.nr_classes; iclass++)
	{
				...
				if (node->myRandomSubset() == isubset)
				{
					...
					for (int recv_node = my_first_recv; recv_node < node->size; recv_node += nr_subsets)
					{
						if (node->rank == reconstruct_rank && recv_node != node->rank)
						{
							...
							node->relion_MPI_Send(&(mymodel.hrise[iclass]), 1, MPI_DOUBLE, recv_node, 0, MPI_COMM_WORLD);
							node->relion_MPI_Send(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, recv_node, 0, MPI_COMM_WORLD);
						...
						else if (node->rank != reconstruct_rank && node->rank == recv_node)
						{
							...
							node->relion_MPI_Recv(&(mymodel.hrise[iclass]), 1, MY_MPI_DOUBLE, reconstruct_rank, 0, MPI_COMM_WORLD, status);
							node->relion_MPI_Recv(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, reconstruct_rank, 0, MPI_COMM_WORLD, status);
		...
		else
		{
			...
			// Broadcast helical parameters
			node->relion_MPI_Bcast(&(mymodel.hrise[iclass]), 1, MY_MPI_DOUBLE, reconstruct_rank, MPI_COMM_WORLD);
			node->relion_MPI_Bcast(&(mymodel.hturn[iclass]), 1, MY_MPI_DOUBLE, reconstruct_rank, MPI_COMM_WORLD);
...
#define DEBUG_FSC
			...
			else if (node->rank == 1)
			{
				...
				if (helix)
					updatePseudoHelicisedFSC(mymodel.fsc_halves_class[iclass], mymodel.hrise[iclass]);
