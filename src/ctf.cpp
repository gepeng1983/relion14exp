...
void CTF::read(MetaDataTable &MD1, MetaDataTable &MD2, long int objectID)
{
	...
	if (!MD1.getValue(EMDL_CTF_PA, PSh, objectID))
		if (!MD2.getValue(EMDL_CTF_PA, PSh, objectID))
			PSh=0;
...
void CTF::setValues(DOUBLE _defU, DOUBLE _defV, DOUBLE _defAng, DOUBLE _voltage,
		DOUBLE _Cs, DOUBLE _Q0, DOUBLE _PSh, DOUBLE _Bfac, DOUBLE _scale)
{
	kV              = _voltage;
	DeltafU         = _defU;
	DeltafV         = _defV;
