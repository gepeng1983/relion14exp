...
#define METADATA_LINE_LENGTH_NOPRIOR 19
#define METADATA_LINE_LENGTH_NOTILT 27
#define METADATA_LINE_LENGTH_ALL 36
...
#define METADATA_CTF_PA 17
#define METADATA_CTF_BFAC 18
...
public:
	...
	/* Search helical symmetry
 	 */
	void hsearch(MultidimArray<double> &img, double &hrise, double &hturn, int myverb=1, const char * log=NULL);

	/* Impose helical symmetry
 	 */
	void himpose(MultidimArray<double> &img, double hrise, double hturn, int myverb=1);

	/* The FSC was determined without imposing helical symmetry
         * Here artifically blow up the SNR curve by square-root number of copies averaged during helicizing
         * and convert to FSC
	 */
	void updatePseudoHelicisedFSC(MultidimArray<double> &fsc, double hrise);
	void updatePseudoHelicisedSSNR(MultidimArray<double> &ssnr, double hrise);
