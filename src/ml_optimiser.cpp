...
void MlOptimiser::parseContinue(int argc, char **argv)
{
	...
	// Can only switch the following two options ON, not OFF
	bool helix1, htune1;
	helix1=parser.checkOption("--helix", "Whether to impose helical symmetry");
	htune1=parser.checkOption("--htune", "Whether to search helical symmetry");
	helix=helix || helix1;
	htune=htune || htune1;
	fnt = parser.getOption("--hinner", "Helical inner radius, in Angstroms", "OLD");
	if (fnt != "OLD")
		hinner=textToFloat(fnt);
	fnt = parser.getOption("--houter", "Helical outer radius, in Angstroms", "OLD");
	if (fnt != "OLD")
		houter=textToFloat(fnt);
	fnt = parser.getOption("--hsuper", "Oversampling factor during imposing helical symmetry, integer", "OLD");
	if (fnt != "OLD")
		hsupersample=textToInteger(fnt);
	fnt = parser.getOption("--htake", "Part of original map taken from center, fraction", "OLD");
	if (fnt != "OLD")
		htake=textToFloat(fnt);
	...
	test2 = textToInteger(getParameter(argc, argv, "--test1", "-999"));
...
void MlOptimiser::parseInitial(int argc, char **argv)
{
	...
	//
	int helix_section = parser.addSection("Helix options");
	helix=parser.checkOption("--helix", "Whether to impose helical symmetry");
	htune=parser.checkOption("--htune", "Whether to search helical symmetry");
	mymodel.inihrise=textToFloat(parser.getOption("--hrise", "Helical Rise per subunit, in Angstroms; minus number to use value from --ref", "1."));
	mymodel.inihturn=textToFloat(parser.getOption("--hturn", "Helical Turn per subunit, in degrees", "1."));
	hinner=textToFloat(parser.getOption("--hinner", "Helical inner radius, in Angstroms", "1."));
	houter=textToFloat(parser.getOption("--houter", "Helical outer radius, in Angstroms", "1."));
	hsupersample=textToInteger(parser.getOption("--hsuper", "Oversampling factor during imposing helical symmetry, integer", "1"));
	htake=textToFloat(parser.getOption("--htake", "Part of original map taken from center, fraction", "0.5"));
	...
	test2 = textToInteger(parser.getOption("--test1", "Special function test1", "-999"));
...
void MlOptimiser::read(FileName fn_in, int rank)
{
    ...
    if (!MD.getValue(EMDL_OPTIMISER_OUTPUT_ROOTNAME, fn_out) ||
		...
		!MD.getValue(EMDL_OPTIMISER_IMPOSE_HELIX, helix)||
		!MD.getValue(EMDL_OPTIMISER_SEARCH_HELIX, htune)||
		!MD.getValue(EMDL_OPTIMISER_HELIX_INNER_RADIUS, hinner)||
		!MD.getValue(EMDL_OPTIMISER_HELIX_OUTER_RADIUS, houter)||
		!MD.getValue(EMDL_OPTIMISER_HELIX_SUPERSAMPLE, hsupersample)||
		!MD.getValue(EMDL_OPTIMISER_HELIX_INCLUDED_PORTION, htake))
...
void MlOptimiser::write(bool do_write_sampling, bool do_write_data, bool do_write_optimiser, bool do_write_model, int random_subset)
{
	...
	if (do_write_optimiser && random_subset < 2)
	{
		...
		MD.setValue(EMDL_OPTIMISER_IMPOSE_HELIX, helix);
		MD.setValue(EMDL_OPTIMISER_SEARCH_HELIX, htune);
		MD.setValue(EMDL_OPTIMISER_HELIX_INNER_RADIUS, hinner);
		MD.setValue(EMDL_OPTIMISER_HELIX_OUTER_RADIUS, houter);
		MD.setValue(EMDL_OPTIMISER_HELIX_SUPERSAMPLE, hsupersample);
		MD.setValue(EMDL_OPTIMISER_HELIX_INCLUDED_PORTION, htake);
...
void MlOptimiser::expectationSetupCheckMemory(bool myverb)
{
	...
	if (myverb > 0)
	{
		std::cout << " Working resolution: " << mymodel.current_size / 2 << std::endl;
		std::cout << " (current_resolution: " << mymodel.getPixelFromResolution(mymodel.current_resolution) << " incr_size: " << incr_size << ")" << std::endl;
	}
...
void MlOptimiser::maximization()
{
	...
	for (int iclass = 0; iclass < mymodel.nr_classes; iclass++)
	{
		if (mymodel.pdf_class[iclass] > 0.)
		{
			...
			FileName fn;
			fn.compose(fn_out+"_it", iter, "", 3);
			fn.compose(fn+"_class", iclass+1, "", 3);
			fn=fn+".hsearch";
			
			if (helix)
				updatePseudoHelicisedSSNR(mymodel.data_vs_prior_class[iclass], mymodel.hrise[iclass]);
			if (htune && !(iter == 1 && do_firstiter_cc))
				hsearch(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], verb, fn.c_str());				
			if (helix)
				himpose(mymodel.Iref[iclass], mymodel.hrise[iclass], mymodel.hturn[iclass], verb);
		...
		if (do_ctf_correction)
		{
			...
			else
			{
				...
				ctf.setValues(DIRECT_A2D_ELEM(exp_metadata, metadata_offset + ipart, METADATA_CTF_DEFOCUS_U),
							  ...
							  DIRECT_A2D_ELEM(exp_metadata, metadata_offset + ipart, METADATA_CTF_PA),
				...
				if (do_ctf_correction)
				{
					...
					else
					{
						...
						ctf.setValues(DIRECT_A2D_ELEM(exp_metadata, my_metadata_entry, METADATA_CTF_DEFOCUS_U),
									  ...
									  DIRECT_A2D_ELEM(exp_metadata, my_metadata_entry, METADATA_CTF_PA),
	...
	for (long int ori_part_id = first_ori_particle_id, my_image_no = 0; ori_part_id <= last_ori_particle_id; ori_part_id++)
    {
		for (long int ipart = 0; ipart < mydata.ori_particles[ori_part_id].particles_id.size(); ipart++, my_image_no++)
		{
			...
			if (do_ctf_correction)
			{
				...
				DOUBLE kV, DeltafU, DeltafV, azimuthal_angle, Cs, Bfac, Q0, PSh;
				...
				if (!mydata.MDimg.getValue(EMDL_CTF_PA, PSh, part_id))
					if (!mydata.MDmic.getValue(EMDL_CTF_PA, PSh, mic_id))
						PSh=0;
				...
				DIRECT_A2D_ELEM(exp_metadata, my_image_no, METADATA_CTF_PA) = PSh;
...
void MlOptimiser::hsearch(MultidimArray<DOUBLE> &img, DOUBLE &hrise, DOUBLE &hturn, int myverb, const char * log)
{
	int hsuper = (hsupersample == 1)?1:hsupersample/2;
	Heliciser<DOUBLE> helix(mymodel.pixel_size, hrise, hturn, hinner, houter, hsuper, htake);

	DOUBLE hriseinc1, hriseinc2, hturninc1, hturninc2;

	std::ofstream * fout;
	if (log) fout = new std::ofstream(log);

	hriseinc1=sampling.getTranslationalSampling(adaptive_oversampling)/ZSIZE(img)*hrise;
	hriseinc2=hriseinc1/10.;

	hturninc1=sampling.getAngularSampling(adaptive_oversampling)/ZSIZE(img)/mymodel.pixel_size*hrise;
	hturninc2=hturninc1/10.;
	

	if (myverb > 0)
		std::cout << std::endl << " Helix: hsearch " << hrise << "(" << hriseinc1 << ")/" << hturn << "(" << hturninc1 << ") @ " << hinner << ":" << houter << "x" << hsuper << " with " << htake << std::endl;

	DOUBLE * logx=new DOUBLE[21*4];
	DOUBLE * logy=new DOUBLE[21*4];

	helix.hsearch(img, hriseinc1, hturninc1, logx, logy);
	...
	if (log)
	{
		for (j=0;j<4;j++)
		{
			(*fout) << "pass " << j << std::endl;
			for (i=-10;i<=10;i++)
				(*fout) << i << " " << logx[j*21+i+10] << " " << logy[j*21+i+10] << std::endl;
		}
	}	

	(*fout) << "Final (1) result (hrise/hturn): " << helix.hrise << "/" << helix.hturn << std::endl;

	if (myverb > 0)
		std::cout << " Helix: hsearch (x10) " << helix.hrise << "(" << hriseinc2 << ")/" << helix.hturn << "(" << hturninc2 << ") @ " << hinner << ":" << houter << "x" << hsuper << " with " << htake << std::endl;

	helix.hsearch(img, hriseinc2, hturninc2, logx, logy);
	...
	(*fout) << "Final (2) result (hrise/hturn): " << helix.hrise << "/" << helix.hturn << std::endl;
...
void MlOptimiser::himpose(MultidimArray<DOUBLE> &img, DOUBLE hrise, DOUBLE hturn, int myverb)
{
	Heliciser<DOUBLE> helix(mymodel.pixel_size, hrise, hturn, hinner, houter, hsupersample, htake);

	if (myverb > 0) 
		std::cout << " Helix: himpose " << hrise << "/" << hturn << " @ " << hinner << ":" << houter << "x" << hsupersample << " with " << htake << std::endl;

	helix.himpose(img);
	
}

void MlOptimiser::updatePseudoHelicisedFSC(MultidimArray<DOUBLE> &fsc, DOUBLE hrise)
{
	DOUBLE deltaz=hrise/mymodel.pixel_size;
	DOUBLE subl=htake*((DOUBLE)mymodel.ori_size)/deltaz;
	long int nsubl = (long int) subl;
	DOUBLE blow = sqrt((DOUBLE)nsubl);

	

	std::cout << " Helix: will average " << nsubl << " fold." << std::endl;
	std::cout << " Helix: blow up SNR by " << blow << " times." << std::endl;

	FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY1D(fsc)
	{
		DOUBLE s, f;
		f=DIRECT_A1D_ELEM(fsc, i);
		...
		// else f=1. unchanged
		DIRECT_A1D_ELEM(fsc, i)=f;
	}

}

void MlOptimiser::updatePseudoHelicisedSSNR(MultidimArray<DOUBLE> &ssnr, DOUBLE hrise)
{
	DOUBLE deltaz=hrise/mymodel.pixel_size;
	DOUBLE subl=htake*((DOUBLE)mymodel.ori_size)/deltaz;
	long int nsubl = (long int) subl;
	DOUBLE blow = sqrt((DOUBLE)nsubl);
	std::cout << " Helix: will average " << nsubl << " fold." << std::endl;
	std::cout << " Helix: blow up SSNR by " << blow << " times. (disabled)" << std::endl;
	
//	FOR_ALL_DIRECT_ELEMENTS_IN_ARRAY1D(ssnr)
//		DIRECT_A1D_ELEM(ssnr, i) *= blow;
}

void test1(MultidimArray<DOUBLE>& in, DOUBLE bfactor, DOUBLE apix)
{

	std::cout << "Engaged in Test1 with " << bfactor/4.0 << "bfactor." << std::endl;

	MultidimArray<DOUBLE> cyl(ZSIZE(in), YSIZE(in)/2, 1440);
	MultidimArray<DOUBLE> cyl2(ZSIZE(in), YSIZE(in)/2, 1440);
	MultidimArray<Complex > Fcyl;

	//Cast in to cylindrical coordinate

	FOR_ALL_ELEMENTS_IN_ARRAY3D(cyl)
	{
		DOUBLE theta= (DOUBLE) j / 720.0 * PI;
		DOUBLE r = (DOUBLE) i;

		DOUBLE x=r*cos(theta);
		DOUBLE y=r*sin(theta);

		DOUBLE kp = k + STARTINGZ(in);

		NZYX_ELEM(cyl, 0, k, i, j) = in.interpolatedElement3D(x, y, kp);
	}
		

	//FFT cyl and apply bfactor
	FourierTransformer tr;
	tr.FourierTransform(cyl, Fcyl);

	cyl.coreDeallocate();

	DOUBLE invsy = apix / YSIZE(Fcyl);

	for (long int k=0; k<ZSIZE(Fcyl); k++)
	for (long int i=0; i<YSIZE(Fcyl); i++)
	{
		long int ip = (i <= YSIZE(Fcyl)/2) ? i : i - YSIZE(Fcyl);
		DOUBLE is=ip*invsy;
		DOUBLE r2s=bfactor/4.0*is*is;
		
		DOUBLE f=exp(r2s);
		//std::cout << "i, ip, is, r2s, f " << i << ", " << ip << ", " << is << ", " << r2s << ", " << f << std::endl;

		for (long int j=0; j<XSIZE(Fcyl); j++)
		{

			DIRECT_A3D_ELEM(Fcyl, k, i, j)*=f;
		}
	}
	
	//Cast Fcyl back to in
	FourierTransformer tr2;
	tr2.inverseFourierTransform(Fcyl, cyl2);
	...
	FOR_ALL_ELEMENTS_IN_ARRAY3D(in)
	{
		DOUBLE jp=j;
		DOUBLE ip=i;
		DOUBLE theta=atan2(ip,jp)/PI*180.0 + 180.0; //TODO REMOVE this magical number
		theta*=4.0;
		DOUBLE r=sqrt(jp*jp+ip*ip);
		...
		NZYX_ELEM(in, 0, k, i, j) = cyl2.interpolatedElement3D(theta, r, k - STARTINGZ(in));
